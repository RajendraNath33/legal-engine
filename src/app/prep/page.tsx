                ...
                <li key={c.id}>
                  <button
                    onClick={() => {
                      ...
                      if (pos >= 0) setIndex(pos); else { setFilter("all"); setIndex(LANDMARK_CASES.findIndex((v) => v.id === c.id)); }
                    }}
                    className={
...
function MockTest() {
  ...
  // Adaptive: after 5 Qs, emphasise wrong topics (just reshuffle)
  const questions = useMemo(() => {
    // Start with all; later we could skew based on weak areas
    ...
    <div className="card mx-auto max-w-3xl p-6">
      ...
      <ul className="mt-6 space-y-2">
        {q.options.map((o, i) => {
          ...
          let cls = "border-slate-800/70 bg-slate-950/40 text-slate-200 hover:border-amber-500/30 light:bg-white light:border-slate-200 light:text-slate-900";
          if (revealed) {
            if (isCorrect) cls = "border-emerald-500/40 bg-emerald-500/10 text-emerald-200";
            else if (isSelected) cls = "border-rose-500/40 bg-rose-500/10 text-rose-200";
            else cls = "border-slate-800/70 bg-slate-900/40 text-slate-400 light:bg-slate-50";
          }
          return (
            <li key={i}>
              <button onClick={() => choose(i)} className={"flex w-full items-center gap-3 rounded-lg border px-4 py-3 text-left text-sm transition " + cls}>
                <span className="flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full border border-current text-[11px] font-semibold">{String.fromCharCode(65 + i)}</span>
      ...
      {revealed && (
        <div className="mt-5 rounded-lg border border-amber-500/20 bg-amber-500/5 p-4 text-sm text-slate-200 light:text-slate-800">
          <div className="font-semibold text-amber-300">Explanation</div>
          <p className="mt-1 text-xs leading-relaxed">{q.explanation}</p>
        </div>
      )}

      <div className="mt-6 flex justify-end">
        <button onClick={next} disabled={!revealed} className="btn-primary">
          {index + 1 === questions.length ? "Finish test" : "Next question"} <ChevronRight className="h-4 w-4" />
        </button>
      </div>
    </div>
  );
...
function topicOf(question: string): string {
  if (/basic structure|kesavananda/i.test(question)) return "Constitution — Basic Structure";
  if (/privacy|puttaswamy/i.test(question)) return "Article 21 — Privacy";
  if (/maneka|passport/i.test(question)) return "Article 21 — Due process";
  if (/cheque|ni act|138/i.test(question)) return "NI Act — Section 138";
  if (/vishaka|harassment|posh/i.test(question)) return "Gender Justice — POSH";
  if (/adultery|497/i.test(question)) return "Constitution — Adultery";
  if (/oleum|hazardous|environment/i.test(question)) return "Environmental Law — Absolute Liability";
  if (/bommai|356/i.test(question)) return "Federalism — Article 356";
  if (/mandal|reservation|obc/i.test(question)) return "Reservation — Indra Sawhney";
  if (/377|same-sex/i.test(question)) return "LGBTQ+ Rights — Sec 377";
  if (/custodial|d\.?k/i.test(question)) return "Criminal Procedure — Arrest";
  if (/speedy trial|hussainara|undertrial/i.test(question)) return "Article 21 — Speedy Trial";
  return "General";
}
